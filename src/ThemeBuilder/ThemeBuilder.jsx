import style from './ThemeBuilder.module.css'
...
const ThemeBuilder = () => {
  // document.title = "Todura - Theme Builder";
  const Sidebar = () => {
    const PickerBg = (prop) => {
      ...
      return (
        <>
          <span>{prop.label}:
            <input className={style.input} onChange={(e) => update(e, prop.jsonKey)} type="text" id={prop.jsonKey} />
          </span><br />
        </>
      )
    }
    const PickerNumber = (prop) => {
      ...
      return (
        <>
          <span>{prop.label}: <input className={style.input} onChange={(e) => update(e, prop.jsonKey)} type="text" id={prop.jsonKey} /></span><br />
        </>
      )
    }
    const Picker = (prop) => {
      ...
      return (
        <>
          <span>{prop.label}: <br />
            Color: <input className={style.colorinput} onChange={(e) => updateColor(e, prop.jsonKey)} type="color" id={prop.jsonKey} />
            <br />Alpha: <input className={style.alphainput} onChange={(e) => updateColorAlpha(e, prop.jsonKey)} type="range" id={`${prop.jsonKey}a`} />
          </span><br /><br />
        </>
      )
    }
    ...
    return (
      <div id={style.preview}>
        <div id={style.navbar}>
          ...
          <div id={style.close}></div>
        </div>
        <div id={style["preview-sidebar"]}>
          <div id={style.intro}></div>
          <div id={style.text1}></div>
          <div id={style.text2}></div>
          <div id={style.button}></div>
        </div>
        <div id={style.item}>
          <div id={style.checkmark}></div>
          <div id={style.itemtitle}></div>
          <div id={style.itemtext}></div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={style.main}>
      <Sidebar />
      <Preview />
      <button id={style.exportButton} onClick={() => exportJson()}>Export to JSON</button>
    </div>
  )
}
